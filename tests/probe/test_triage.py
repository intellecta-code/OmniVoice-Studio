...
def test_detect_repo_from_origin():
    # This repo's origin is github.com/<owner>/OmniVoice-Studio.
    repo = T.detect_repo()
    assert repo is not None
    assert repo[1] == "OmniVoice-Studio"
...
def test_triage_builds_github_url():
    ...
    assert res.repo == "OmniVoice-Studio"
    assert res.url and res.url.startswith(
        f"https://github.com/{res.owner}/OmniVoice-Studio/issues/new?"
    )
